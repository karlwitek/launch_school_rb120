...
# I moved attr_reader :serial_number out of 'private' and made it public. Worked, but not correct.
# LS: Private methods cannot be invoked with an explicit caller, even inside their own class.
# In Employee#==, we invoke serial_number with an explicit caller(other).
# In order to work, we make serial_number a protected method. Protected work like private(from
# outside the class), from inside the class, protected methods are accessible and may be invoked
# with an explicit caller. Review Book..
